...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, firstIndex) + str.slice(firstIndex + lenValue);
}
...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, lastIndex) + str.slice(lastIndex + lenValue);
}
...
function isPalindrome(str) {
  ...
  const lenStr = strLower.length;
  for (let i = 0; i < Math.round(lenStr / 2); i += 1) {
    if (strLower[i] === strLower[lenStr - 1 - i]) {
      isPal += 1;
    }
  }
  if (isPal === Math.round(lenStr / 2)) {
    return true;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < listSentence.length; i += 1) {
    if (maxWordLength < listSentence[i].length) {
      maxWordLength = listSentence[i].length;
      maxWord = listSentence[i];
    }
  }
  return maxWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < str.split(' ').length; i += 1) {
    result += str.split(' ')[i].split('').reverse().join('').concat(' ');
  }
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    let nextCharCode = charCode + 13;
    if (charCode >= 65 && charCode <= 90) {
      if (nextCharCode > 90) {
        nextCharCode = (nextCharCode % 90) + 64;
      }
